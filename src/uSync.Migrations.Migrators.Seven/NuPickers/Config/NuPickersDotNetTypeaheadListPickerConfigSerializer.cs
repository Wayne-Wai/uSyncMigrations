...
internal class NuPickersDotNetTypeaheadListPickerConfigSerializer : NuPickersConfigurationDataListBase, IConfigurationSerializer
{
    public NuPickersDotNetTypeaheadListPickerConfigSerializer(IOptions<NuPickerMigrationOptions> options)
        : base(options)
    { }

    public string Name => nameof(NuPickersDotNetTypeaheadListPickerConfigSerializer);
    public string[] Editors => ["nuPickers.DotNetTypeaheadListPicker"];
    ...
    public override Task<IDictionary<string, object>> GetMigratedConfigurationAsync(string name, IDictionary<string, object> configuration)
    {
        ...
        if (configuration.TryGetValueAsObject<NuPickersDotNetTypeConfig>("dataSource", out var nuPickersConfig) is false)
        ...
        var dataSource = new[]
        {
            ...
            {
                key =
                    $"{MapNamespace(nuPickersConfig?.ClassName)}, {MapAssembly(nuPickersConfig?.AssemblyName)}",
                value = ""
            }
        }.ToList();
